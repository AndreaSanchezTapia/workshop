...
A metodologia proposta para identificar os sítios resilientes às mudanças climáticas [@anderson_estimating_2014; @anderson_resilient_2016; @anderson_resilient_2016-1; e @anderson_resilient_2023] envolve a divisão do espaço geográfico em regiões eco-geológicas, que são similares na sua geologia, geomorfologia, vulnerabilidade ao intemperismo, formação de solos e biodiversidade. Dentro de cada região são mapeadas a diversidade da paisagem e a conectividade local, que posteriomente, formam a resiliência da paisagem.
...
Para estimar a resiliência de um sítio, é necessário estimar a [__diversidade da paisagem__](#diversidade-da-paisagem) e a [__conectividade local__](#conectividade-local) para cada célula de 90 metros.
...
A __diversidade da paisagem__ é calculada pelo número de _landforms_ dentro de uma área circular de 450 metros no entorno de cada célula de 90 metros.


#### Amplitude altitudinal

A amplitude altitudinal é determinada a partir do DEM, tabulando a amplitude em uma área circular de 450 metros no entorno de cada célula de 90 metros.
A amplitude altitudinal considerada no estudo corresponde ao componente não correlacionado (os resíduos de uma regressão linear simples) com a diversidade de _landforms_.
...
A partir de um mapa de uso e cobertura do solo, incluindo a presença de estradas, foi realizada uma simplificação dos tipos de uso em componentes principais da paisagem e determinado pesos de resistência para cada categoria, nos quais áreas naturais apresentam o valor mínimo, e áreas com intervenção antrópica intensa, o valor máximo. 
...
O mapeamento final dos sítios resilientes é feito por [__configurações eco-geológicas__](#configurações-eco-geológicas) (item acima) dentro de ecoregiões.
