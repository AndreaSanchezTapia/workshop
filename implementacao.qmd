...
Para o cálculo de  [inclinação](#inclinação_do_relevo), [aspecto](#aspecto_do_relevo) e [índice de posição topográfica (TPI)](#índice_de_posição_topográfica), utilizamos a imagem do modelo digital de elevação (DEM) do Merit-DEM [@citação], em uma escala de 90 metros. Escolhemos utilizar esse DEM por ser um produto em escala global, de livre acesso e com correções de vários viéses de modelos de elevação, que possibilitam que nosso trabalho seja replicável e mais próximo a realidade dos biomas que modelamos [@citação].
...
|43 | Cool Footslope  |
|44 |Warm Sideslope|
: Códigos das _landforms_ obtidas após classificação do relevo {#tbl-landforms}
...
Dessa forma, temos uma imagem que nos mostra a variação, em relação à média, dos valores dos resíduos da amplitude altitudinal para o Brasil e dentro de cada uma das categorias de **regiões eco-geológicas**.
...
$$
Ao final de todas as operações, temos uma única camada com valores que representam a variação em relação à média dos valores de densidade e contagem de pixels de áreas de banhado tanto para a extensão do Brasil, quanto referentes às regiões eco-geológicas.
